...
def star_wars
  ...
  # hint: use 'select' and 'where'
  movies = Movie.where('title LIKE ?', '%Star Wars%').select(:id, :title, :yr)
end
...
def below_average_years
  ...
  Movie.select(:yr, 'count(yr) AS bad_movies')
    .where('score < ?', 5).group(:yr).order('count(yr) DESC')

    # Select yr, count(yr) FROM movies
    # WHERE score < 5
    # Group_by yr
    # order by count(yr) DESC
end
